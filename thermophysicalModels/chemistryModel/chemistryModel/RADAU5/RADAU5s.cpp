...
// Constructor
RADAU5s::RADAU5s(const int nin, double yin[], double xin, double xendin,
	...
	RADAU5(nin, yin, xin, xendin, dxin, itolerin, rtolerin, atolerin, ioutin,
...
RADAU5s::RADAU5s(const int nin, double yin[], double xin, double xendin,
	...
	int mlmasin, int mumasin): RADAU5(nin, yin, xin, xendin, dxin),
	ijac(ijacin), mljac(mljacin), mujac(mujacin), imas(imasin), mlmas(mlmasin),
...
void RADAU5s::ResetT(double yin[], double xin, double xendin,
	double dxin, int itolerin, double *rtolerin, double *atolerin, 
	double hin, double hmaxin, int nmaxin, double uroundin, double safein,
	double faclin, double facrin, int mljacin, int mujacin,
	int mlmasin, int mumasin, int nitin, bool startnin,
	int nind1in, int nind2in,
	int nind3in, int npredin, int m1in, int m2in, bool hessin, double fnewtin,
	double quot1in, double quot2in, double thetin) 
...
// Computation of the row-dimensions of the 2-D arrays
	...
	//cout << "Integrator dimension:  " << n << endl;	
...
// Allocate memory for 2-D arrays
	...
	{
		cout << "something wrong  in ResetT() ..." <<  endl; 
	}

} // ResetT

void RADAU5s::Integrate()
{

	//cout << "RADAU5s/CoreIntegrator() ... " << endl;
...
// constants
	...
	// cout << "RADAU5mass() ..." << endl;
	if (implct) RADAU5mass(fmas);
	...
	//cout << "RADAU5derivative() ..." << endl;
	RADAU5derivative(x, y, y0);
	nfcn++;
	...
	//cout << "RADAU5s/ComputeJacobian() ..." << endl;
	ComputeJacobian();
	//cout << "RADAU5s/ComputeJacobian() ...done" << endl;
	...
	while (loop) {
		...
		//cout << "RADAU5s/DecompReal() ... " << zzz++ <<  endl;
		ier = DecompReal();
		...
		//cout << "RADAU5s/DecompComplex() ..." << endl;
		ier = DecompComplex();
		...
		while (true) {
			...
			while (true) {
				...
				for (int i = 0; i < n; i++) cont[i] = y[i] + z1[i];
					RADAU5derivative(x+c1*h, cont, z1);

				for (int i = 0; i < n; i++) cont[i] = y[i] + z2[i];
					RADAU5derivative(x+c2*h, cont, z2);

				for (int i = 0; i < n; i++) cont[i] = y[i] + z3[i];
					RADAU5derivative(xph, cont, z3);
			...
			// error estimation
			err = 0.0;
			//cout << "RADAU5s/ErrorEstimate() ..." << endl;
			ier = ErrorEstimate();
			if (ier == -1) return -1;
	...
	//cout << "RADAU5s/CoreIntegrator() ...done " << endl;
	return 1;
}  // CoreIntegrator

void RADAU5s::ComputeJacobian()
{	
	//cout << "RADAU5s/ComputeJacobian() ... " << endl;
	njac++;
	if (ijac == 0) {
	...
	// compute Jacobian matrix analytically
		RADAU5jacobian(x, y, fjac);
	}

	caljac = true;
	calhes = true;
	//cout << "RADAU5s/ComputeJacobian() ...f... " << endl;
	return;
}

double RADAU5s::ContinuousOutput(unsigned i)
{
